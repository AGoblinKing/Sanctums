...
def('Jxl.State', {
    ...
    add: function() {
        for(var i = 0; i < arguments.length; i++) {
           this.defaultGroup.add(arguments[i]); 
        }
        return this;
    },
...
def('Jxl.Sprite', {
    ...
    loadGraphic: function(params) {
        ...
        this.width = this.width === 0 ? this.graphic.width : this.width;
        this.height = this.height === 0 ? this.graphic.height : this.height;
    ...
    createGraphic: function(Width, Height, Color) {
        ...
        this.width = this.frameWidth = this.graphic.width;
	    this.height = this.frameHeight = this.graphic.height;
    ...
    solveXCollision: function(obj1, obj2) {
        ...
        for (i1 = 0; i1 < l1; i1++) {
            ...
            for (i2 = 0; i2 < l2; i2++) {
                ...
                if (p1hn2) {
                    obj1.hitRight(obj2, sv2);
                    obj2.hitLeft(obj1, sv1);
                }
                else {
                    obj1.hitLeft(obj2, sv2);
                    obj2.hitRight(obj1, sv1);
                }
                obj1.hitSide(obj2, sv2);
                obj2.hitSide(obj1, sv1);
                obj1.hit(obj2, sv2);
                obj2.hit(obj1, sv1);
...
def('Jxl.Mouse', {
    ...
    init: function() {
        ...
        Jxl.canvas.addEventListener('mousemove', function(e) {
            self.x = e.clientX/Jxl.scale.x-Jxl.scroll.x;
            self.y = e.clientY/Jxl.scale.y-Jxl.scroll.y;
        }, true);
        
        Jxl.canvas.addEventListener("mousedown", function() {
            self.click = true;
        });
        
        Jxl.canvas.addEventListener("mouseup", function() {
            self.click = false;
        });
...
def('Jxl.Keyboard', {
    init: function() {
        ...
        window.addEventListener('keyup', function(e) {
            e.preventDefault();
            self.ups[e.keyCode] = true;
            self.ups[e.keyCode] = String.fromCharCode(e.keyCode);
    ...
    update: function() {
        ...
        Object.keys(this.ups).forEach(function(up) { up = false; });
